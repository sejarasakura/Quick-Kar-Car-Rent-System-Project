...
<html>
    <head>
        ...
        <%
            ...
            String year_of_drive = request.getParameter("year_of_drive");
            String card_num = request.getParameter("card_num");
            domain.setFirst_name(first_name);
            domain.setLast_name(last_name);
            domain.setIc_num(ic_number);
            domain.setLicense(license);
            domain.setPassword(password);
            domain.setYear_of_drive(Integer.parseInt(year_of_drive));
            domain.setPhone_num(phone_num);
            domain.setDriver_id(_control.GenerateID.generate_Driver(year_of_drive, false));
            domain2.setPay_method(_enum.PaymentMethodType.getValue(payment_method));
            domain2.setCard_num(card_num);
            domain2.setUser(domain.getDriver_id());
            ...
            updated = da1.addRecord(domain);
            updated &= da2.addRecord(domain2);
            request.getSession().setAttribute("email-" + domain.getDriver_id(), email);
        %>
    </head>
    <body>
        <div id="header"></div>
        ...
        <%
            response.sendRedirect("http://localhost:8080/GroupProject/_page/error/error_page_design.jsp?"
                    + ErrorDetails.E009.getUrl() + "&url=addDriver");      
        %>
        <%}%>
        <div id="footer"></div> 
    </body>
